...
import { editDocKramdown, editBlockKramdown } from '../utils/markdown.js';
import { putFile } from './../utils/api.js';
import { stat, copyFile, rm } from './../utils/system.js';
import {
    getFocusedID,
    getTargetBlockID,
    getTargetHistory,
        ...
        if (config.theme.window.enable) {
            ...
            if (config.theme.window.open.enable) {
                ...
                if (config.theme.window.open.panel.enable) {
                    // 打开一个新窗口
                    toolbarItemInit(config.theme.window.open.panel.toolbar, () => {
                        const windowParams = merge({}, config.theme.window.windowParams, {
                            width: 1080, // 窗口宽度
                            height: 720, // 窗口高度
                            center: true, // 居中显示
                                        ...
                                        middleClick(
                                            ...
                                            async href => {
                                                if (href.startsWith('file:')) {
                                                    ...
                                                    let path = config.theme.regs.winpath.test(
                                                        url.pathname
                                                    )
                                                    ...
                                                    if (stats) {
                                                        ...
                                                        await copyFile(
                                                            ...
                                                            window.theme.win =
                                                                window.theme.openNewWindow(
                                                                    'editor',
                                                                    config.theme.window.open.editor
                                                                        .path.index,
                                                                    {
                                                                        mode: 'local',
                                                                        url: encodeURI(path),
                                                                        path: encodeURI(
                                                                            temp_file_path_relative
                                                                        ),
                                                                        lang: window.theme
                                                                            .languageMode,
                                                                        // theme: window.siyuan.config.appearance.mode,
                                                ...
                                                putFile(
                                                    ...
                                                    if (r && r.code === 0) {
                                                        ...
                                                        window.theme.win =
                                                            window.theme.openNewWindow(
                                                                'editor',
                                                                config.theme.window.open.editor.path
                                                                    .index,
                                                                {
                                                                    mode: 'inbox',
                                                                    url: encodeURI(
                                            ...
                                            async snippet => {
                                                ...
                                                const urlParams = {
                                                    ...
                                                    mode: 'snippet',
                                                    url: encodeURI(
                                                        `${window.siyuan.config.system.workspaceDir}/data/snippets/conf.json`.replaceAll(
                                                            '\\',
                                                            '/'
                                                        )
                                                    ),
