...
import { getEditor, getDockFromPanel, getFocusedDocID } from './../utils/dom.js';
import { ialCreate, HTMLDecode } from './../utils/string.js';
import { getObjectLength, copyToClipboard } from './../utils/misc.js';
...
async function docCut() {
    ...
    if (id) {
        ...
        if (data) {
            ...
            copyToClipboard(content)
                .then(async () => {
                    const response = await updateBlock(id, 'markdown', '');
    ...
    outline: {
        style: {
            ...
            content: {
                ...
                link: (text, id, title) =>
                    `[${text}](siyuan://blocks/${id}${title ? ` "${title}" ` : ''})`,
...
async function outlineCopy(mode) {
        ...
        switch (node.localName) {
            case 'li':
                ...
                if (indent >= 0) {
                    ...
                    let text =
                        node.querySelector('span.b3-list-item__text').innerText ||
                    ...
                    markdown.push(
                        `${' '.repeat(indent)}${mark(++index)}${heading.handler(
                            level,
                            heading.enable
                        )}${content(text, id)}`
                    );
    ...
    function outlineParser(node, offset = 0, index = 0) {
        ...
        if (indent >= 0) {
            // 节点信息
            let id = node.id; // 块 ID
            let text = HTMLDecode(node.content || node.name || config.theme.doc.outline.empty); // 块内容
            let level = node.subType ? parseInt(/^h(\d+)$/.exec(node.subType)[1]) : 0; // 标题级别
            let heading = config.theme.doc.outline.heading; // 标题级别标志配置选项
            markdown.push(
                `${' '.repeat(indent)}${mark(++index)}${heading.handler(
                    level,
    ...
    if (markdown.length > 0) {
        if (getObjectLength(config.theme.doc.outline.ial) > 0)
            markdown.push(ialCreate(config.theme.doc.outline.ial));
    ...
    if (editor) {
        ...
        for (const index of sequence) {
            const headings = await sql(
                `SELECT id FROM blocks WHERE root_id = '${id}' AND type = 'h' AND subtype = 'h${index}';`
            );
    ...
    try {
        if (config.theme.doc.enable) {
            ...
            if (config.theme.doc.copy.enable) {
                // 复制当前文档全文
                const Fn_docCopy = toolbarItemInit(config.theme.doc.copy.toolbar, docCopy);
                globalEventHandler.addEventHandler('keyup', config.theme.hotkeys.doc.copy, _ =>
                    Fn_docCopy()
                );
            }
            ...
            if (config.theme.doc.delete.enable) {
                // 删除当前文档全文
                const Fn_docDelete = toolbarItemInit(config.theme.doc.delete.toolbar, docDelete);
                globalEventHandler.addEventHandler('keyup', config.theme.hotkeys.doc.delete, _ =>
                    Fn_docDelete()
                );
            }
            ...
            if (config.theme.doc.cut.enable) {
                // 剪切当前文档全文
                const Fn_docCut = toolbarItemInit(config.theme.doc.cut.toolbar, docCut);
                globalEventHandler.addEventHandler('keyup', config.theme.hotkeys.doc.cut, _ =>
                    Fn_docCut()
                );
            }
        }
