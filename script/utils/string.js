...
function compareVersion(version1, version2) {
    ...
    for (let i = 0; i < v1_arr.length; i++) {
        ...
        let v1, v2;
        if (!isNaN(v1_str) && !isNaN(v2_str)) {
            // 两者都为数字
            v1 = v1_int;
            v2 = v2_int;
        } else if (!isNaN(v1_arr[i]) || !isNaN(v2_arr[i])) {
            // 其中一者为数字
            if (v1_int !== v2_int) {
                // 版本号不一致
                v1 = v1_int;
                v2 = v2_int;
            } else if (!isNaN(v1_str) && isNaN(v2_str))
                // v1 是正式版 | v2 是内测(x-alphaX)/公测(x-betaX)/开发版(x-devX)
                return 1;
            else if (isNaN(v1_str) && !isNaN(v2_str))
                // v2 是正式版 | v1 是内测(x-alphaX)/公测(x-betaX)/开发版(x-devX)
                return -1;
            // 意外的情况
            else return 0;
        } else {
            // 都不为数字, 比较字符串, 开发版(dev) < 内测版(alpha) < 公测版(beta)版本号小
            if (v1_int !== v2_int) {
                // 版本号不一致
                v1 = v1_int;
                v2 = v2_int;
