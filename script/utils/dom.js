...
function getTargetDocID(block) {
    ...
    return content?.getElementsByClassName('protyle-background')?.[0]?.dataset?.nodeId ?? null;
}
...
function getTargetBlock(target) {
    let element = target;
    while (element && element.dataset && !config.theme.regs.id.test(element.dataset.nodeId))
        element = element.parentNode;
    if (element && element.dataset && config.theme.regs.id.test(element.dataset.nodeId))
        return element;
    else return null;
}
...
async function getTargetBlockIndex(target) {
    ...
    let top = element;
    while (
        element != null && // 没有找到目标, 退出
        !element.classList.contains('protyle-wysiwyg') // 如果没有找到目标, 退出
    )
        (top = element), (element = element.parentElement);

    if (element) {
        const scroll = top.parentElement.parentElement.querySelector('protyle-scroll__bar'); // 块滚动条
        ...
        if (!index || index > MAX) {
            ...
            if (offset ? first_index < MAX : first_index <= MAX) {
                ...
                first_index = null;
                while (true) {
                    if (pre_pointer || next_pointer) {
                        // 如果有指针, 则继续
                        if (pre_pointer && parseInt(pre_pointer.dataset.nodeIndex) <= MAX) {
                            // 搜索到 index 处于合理区间的块
                            first_index = pre_pointer.dataset.nodeIndex;
                            break;
                        }
                        if (next_pointer && parseInt(next_pointer.dataset.nodeIndex) <= MAX) {
                            // 搜索到 index 处于合理区间的块
        ...
        return {
            doc: top.parentElement.parentElement.firstElementChild.dataset.nodeId,
            index: index,
            offset: offset,
            scroll: scroll,
        };
...
function getTargetInboxID(target) {
    let element = target;
    while (element != null && !config.theme.regs.inboxid.test(element.dataset.id))
        element = element.parentElement;
...
function getTargetSnapshotDoc(target) {
    ...
    if (element != null) {
        let node = element;
        while (node != null && node.classList.contains('b3-dialog__diff'))
...
function getTargetHref(target) {
    let href = null;
    if (target.localName === 'span' && target.dataset.href) href = target.dataset.href;
    if (target.localName === 'a' && target.href) href = target.href;
    return href;
}
...
.b3-typography:not(.b3-typography--default), .protyle-wysiwyg, .protyle-title, .protyle-title__input{font-family: "${
            window.siyuan.config.editor.fontFamily
        }", "quote", "Helvetica Neue", "Luxi Sans", "DejaVu Sans", "Hiragino Sans GB", "Microsoft Yahei", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", "Segoe UI Symbol", "Android Emoji", "EmojiSymbols" !important;}
`;
...
function getEditors() {
    return window.siyuan.layout
        ? getEditorsFromLayout(window.siyuan.layout.centerLayout).concat(
              getEditorsFromblockPanels(window.siyuan.blockPanels)
          )
...
function enableProtyle(protyle) {
    ...
    if (
        ...
        ['MacIntel', 'iPhone'].includes(navigator.platform)
    ) {
    ...
    protyle.wysiwyg.element
        .querySelectorAll('[contenteditable="false"][spellcheck="false"]')
        .forEach(item => {
