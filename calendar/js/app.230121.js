                ...
                methods: {
                    ...
                    async parseNotePath() {
                        ...
                        if (0 === nb_conf.code && nb_conf.data) {
                            ...
                            var dayPath = notePathSplit[2]
                                .replace('now | date "', '')
                                .replace('"}}', ''); // -> 2006.01.02
                            this.dayCurrent = dayPath
                                .replaceAll('2006', '${e[0]}')
                                .replaceAll('01', '${e[1]}')
                                .replaceAll('02', '${e[2]}'); // -> '${e[0]}.${e[1]}.${e[2]}'

                            console.log(
                                '[日历插件][info] 替换为当前日期的模板变量',
                                this.dailyNotePath +
                                    '/' +
                                    this.monthCurrent +
                                    '/' +
                                    this.dayCurrent,
                                '其中e为形如[2021, 10, 27]的当前日期变量'
                            );
                    ...
                    async SiYuan_SQL_dailyNote(t) {
                        let url = '/api/query/sql',
                            data = {
                                stmt: `select * from blocks  WHERE content LIKE '${t}' AND type = 'd' ORDER BY random() LIMIT 1`,
                            },
                            resData = null;
                        return (
                            await fetch(url, {
