                    ...
                    [
                        _c('div', { staticClass: 'now-data-myself' }, [
                            _c(
                                ...
                                {
                                    ref: 'today',
                                    staticClass: 'now-data-myself-time',
                                    attrs: { title: '刷新' },
                                },
                                [_vm._v(' ' + _vm._s(_vm.date) + '日 ')]
                            ),
                            _c(
                                'div',
                                {
                                    ref: 'week',
                                    staticClass: 'now-data-myself-week',
                                    attrs: { title: _vm.currentNotebook.name },
                                },
                            ...
                            _vm.showNotebookList
                                ? _c(
                                      'div',
                                      {
                                          staticClass: 'notebooksList',
                                          on: { click: _vm.handleNotebookClick },
                                      },
                                      _vm._l(_vm.notebookList, function (o) {
                                          return _c(
                                              'div',
                                              {
                                                  ...
                                                  attrs: { 'data-id': o.id, 'data-name': o.name },
                                              },
                        ...
                        _c('Calendar', {
                            ref: 'Calendar',
                            attrs: { markDate: _vm.markArr, textTop: _vm.textTop },
                            on: { choseDay: _vm.clickDay, changeMonth: _vm.changeDate },
                        }),
                    ],
                    1
                ),
                ...
                methods: {
                    ...
                    async handleNotebookClick(e) {
                        if (e.target.classList.contains('notebook')) {
                            ...
                            console.log(
                                '[日历插件][Info] Switch currentNotebook to [' +
                    ...
                    async get_crt_nb() {
                        try {
                            ...
                            console.log(
                                '[日历插件][Error] 大概率是由于第一次启动，配置文件 [' +
                                    this.config_json_path +
                                    '] 未创建，文件不存在造成的404, 错误日志如下:\n',
                                err
                            );
                    ...
                    async set_crt_nb() {
                        await this.写入文件(
                            this.config_json_path,
                            JSON.stringify(this.currentNotebook, undefined, 4)
                        );
                    },
                    ...
                    async init_open_app() {
                        ...
                        if (this.currentNotebook.name !== '') {
                            ...
                            // print log info
                            console.log(
                                '[日历插件][Info] 未在缓存中找到默认笔记本设置，使用笔记本[' +
                                    this.currentNotebook.name +
                                    ']作为默认'
                            );
                            let success_str = {
                                msg:
                                    '[日历插件][info] 未在缓存中找到默认笔记本设置，使用笔记本[' +
                                    this.currentNotebook.name +
                                    ']作为默认，如有需要请点击日历的【星期x】按钮进行切换',
                                timeout: 7000,
                            };
                            let print = await this.request(
                    ...
                    async parseNotePath() {
                        ...
                        if (0 === nb_conf.code && nb_conf.data) {
                            ...
                            this.notePathParse = notePath.replace(/\{\{(.*?)\}\}/g, y =>
                                y.replace(
                                    /\bnow\b(?=(?:(?:[^"]*"){2})*[^"]*$)/g,
                            ...
                            var monthPath = notePathSplit[1]
                                .replace('now | date "', '')
                                .replace('"}}', ''); // -> 2006/2006.01
                            this.monthCurrent = monthPath
                                .replaceAll('2006', '${e[0]}')
                                .replaceAll('01', '${e[1]}'); // -> '${e[0]}/${e[0]}.${e[1]}'

                            var dayPath = notePathSplit[2]
                                .replace('now | date "', '')
                                .replace('"}}', ''); // -> 2006.01.02
                            // this.dayCurrent = notePath.split('/{{');
                            ...
                            if (systemConf && systemConf.data.conf.system.workspaceDir) {
                                ...
                                this.template_path =
                                    `${workspaceDir}/data/templates${nb_conf.data.conf.dailyNoteTemplatePath}`.replace(
                                        /\//g,
                                        '\\'
                                    );
                            } else {
                            ...
                            let error_str = {
                                msg: '[日历插件] 读取用户配置失败，请点击日历面板->星期，选择对应的笔记本后再试',
                                timeout: 7000,
                            };
                            let print = await this.request(
                                '/api/notification/pushErrMsg',
                            ...
                            console.log(
                                '[日历插件][Error] 读取用户配置失败，请点击日历面板->星期，选择对应的笔记本后再试',
                                nb_conf.data
                            );
                        }
                    },
                    ...
                    async clickDay(data) {
                        ...
                        let TempData = eval('`${e[0]}-${e[1]}-${e[2]}`'); // -> 2021-10-22
                        ...
                        if (this.DateLinkToNote[TempData]) {
                            ...
                            if (this.currentNotebook.id) {
                                ...
                                let notePathGO = this.notePathParse.replaceAll(
                                    '[[dateSlot]]',
                                    TempData
                                );
                                let notePathRendered = await this.renderSprig(notePathGO);
                                let createDocWithMdReqBody = {
                                    notebook: this.currentNotebook.id,
                                    path: notePathRendered,
                                    markdown: '',
                                };
                    ...
                    async renderSprig(t) {
                        ...
                        try {
                            const response = await fetch(url, {
                                ...
                                headers: {
                                    'Content-Type': 'application/json',
                                    Authorization: 'Token YOUR_API_TOKEN', // 替换 YOUR_API_TOKEN 为你的实际API令牌
                                },
                            });
                            const responseData = await response.json(); // 将响应转换为JSON
                            if (responseData.code === 0) {
                                // console.log('Rendered path:', responseData.data);
                                return responseData.data; // 返回解析后的路径
                            } else {
                                console.error('Error:', responseData.msg);
                                return null;
                            }
                        } catch (error) {
                            console.log('[日历插件][Error]', error);
                            return null;
                        }
                    },
                    async SiYuan_SQL_dailyNote(t) {
                        let url = '/api/query/sql',
                            data = {
                                stmt: `select * from blocks  WHERE hpath LIKE '${t}' AND type = 'd' ORDER BY random() LIMIT 1`,
                            },
                            resData = null;
                        return (
                            await fetch(url, {
                    ...
                    async AutoMarkDate(data) {
                        // 在日历中以下方小红点展示已有日记
                        let notePathGO = this.notePathParse.replaceAll('[[dateSlot]]', data);
                        let notePathRendered = await this.renderSprig(notePathGO);
                        let res = await this.SiYuan_SQL_dailyNote(notePathRendered);
                        // console.log(res)
